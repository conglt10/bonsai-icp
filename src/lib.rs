...
fn mint_batch(
    ...
    blob_content: Vec<u8>,
) -> Result<BatchMintResult, ConstrainedError> {
    let txid = STATE.with(|state| {
        let mut state = state.borrow_mut();
        for _n in 1..=_amount {
            let new_id = state.nfts.len() as u64;
            let nft = Nft {
...
async fn summon(
    ...
    let account = AccountIdentifier::new(&canister_id, &principal_to_subaccount(&api::caller()));
    ...
    if balance.e8s() < ICP_FEE + _amount * PRICE {
        return Err(DepositErr::BalanceLow);
    }

    let transfer_args = ic_ledger_types::TransferArgs {
        memo: Memo(0),
        amount: Tokens::from_e8s(_amount * PRICE),
        fee: Tokens::from_e8s(ICP_FEE),
        from_subaccount: Some(principal_to_subaccount(&api::caller())),
    ...
    ic_ledger_types::transfer(ledger_canister_id, transfer_args)
        ...
        .map_err(|_| DepositErr::TransferFailure)?
        .map_err(|_| DepositErr::TransferFailure)?;
    mint_batch(api::caller(), _amount, _metadata, blob_content);
    Ok((_amount * PRICE).into())
}
